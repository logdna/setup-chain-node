...
const {string, array} = require('@answerbook/stdlib')
...
class LookupVisitor extends Visitor {
  ...
  dispatch(ctx) {
    if (ctx.fn) return this.visit(ctx.fn)
    if (ctx.lookup) return this.visit(ctx.lookup)
    return this.scalar(getArg(ctx))
  }

  lookup(ctx) {
    const [node] = (ctx.objectpath || ctx.identifier)
    return {
      type: 'lookup'
    , value: node.image
    , position: position(ctx.hash[0], node)
    }
  }
...
function getArg(ctx) {
  if (ctx.number) return ctx.number[0]
